...
export default function AdminReviewsPage() {
  ...
  const [allReviews, setAllReviews] = useState<Review[]>([...REVIEWS, ...storedReviews]);
  const [replyText, setReplyText] = useState('');
  const [replyTarget, setReplyTarget] = useState<string | null>(null);
  const [filter, setFilter] = useState<Review['status'] | 'all'>('all');

  const refresh = () => setAllReviews([...REVIEWS, ...getReviews()]);

  const filtered = allReviews.filter(r => filter === 'all' || r.status === filter);
  ...
  const handleReply = (id: string) => {
    ...
    updateReviewStatus(id, 'published', replyText);
    refresh();
    setReplyText('');
    setReplyTarget(null);
          ...
          <div key={review.id} className="bg-gray-800 rounded-xl border border-gray-700/50 p-5">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-amber-900/50 flex items-center justify-center text-amber-400 font-semibold flex-shrink-0">
                  {review.customerName[0]}
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-200">{review.customerName}</p>
                  <div className="flex gap-0.5 mt-0.5">
                    {[1,2,3,4,5].map(i => <Star key={i} size={12} className={i <= review.rating ? 'text-amber-400 fill-amber-400' : 'text-gray-600'} />)}
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-gray-700/50">
              {review.status !== 'published' && (
                <button onClick={() => handleStatus(review.id, 'published')} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-green-900/40 text-green-400 hover:bg-green-900/60 text-xs transition-colors"><Check size={12} /> نشر</button>
              )}
              {review.status !== 'rejected' && (
                <button onClick={() => handleStatus(review.id, 'rejected')} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-red-900/40 text-red-400 hover:bg-red-900/60 text-xs transition-colors"><X size={12} /> رفض</button>
              )}
