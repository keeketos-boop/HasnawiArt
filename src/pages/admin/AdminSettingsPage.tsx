...
import { fetchActivityLog, exportData, adminSignOut } from '@/lib/api';
import type { ActivityLog } from '@/types';
import { Check, Download, Key, Bell, Shield, Clock, Eye, EyeOff, Loader2 } from 'lucide-react';
...
export default function AdminSettingsPage() {
  ...
  useEffect(() => {
    if (activeTab === 'activity') {
      setLoadingLog(true);
      fetchActivityLog().then(log => { setActivityLog(log); setLoadingLog(false); });
    }
  }, [activeTab]);
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1 overflow-x-auto scrollbar-hide bg-gray-800 rounded-xl p-1 border border-gray-700/50">
        {tabs.map(({ id, label, Icon }) => (
          <button key={id} onClick={() => setActiveTab(id)} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs transition-all whitespace-nowrap ${activeTab === id ? 'bg-amber-600 text-gray-900 font-medium' : 'text-gray-400 hover:text-gray-200'}`}>
            <Icon size={13} />{label}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'account' && (
        <div className="bg-gray-800 rounded-xl border border-gray-700/50 p-5 space-y-4">
          ...
          <p className="text-xs text-gray-400">كلمة المرور تُدار عبر Supabase Auth. لتغييرها، انتقل إلى: Supabase Dashboard → Authentication → Users</p>
          <div className="max-w-sm space-y-3">
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">كلمة المرور الجديدة</label>
              <div className="relative">
                <input type={showPw ? 'text' : 'password'} value={newPw} onChange={e => setNewPw(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 pr-10 py-2.5 text-gray-200 text-sm focus:outline-none focus:border-amber-500/60" placeholder="••••••••" />
                <button onClick={() => setShowPw(!showPw)} className="absolute top-1/2 right-3 -translate-y-1/2 text-gray-500">{showPw ? <EyeOff size={14} /> : <Eye size={14} />}</button>
              </div>
            </div>
            <div>
            ...
            <button onClick={() => toast.info('لتغيير كلمة المرور، يرجى تعديلها من Supabase Dashboard مباشرة.')} className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-amber-600 hover:bg-amber-500 text-gray-900 text-sm font-medium">
              <Check size={14} /> حفظ
            </button>
          </div>
          <div className="pt-4 border-t border-gray-700/50">
            <button onClick={() => { adminSignOut(); window.location.reload(); }} className="px-4 py-2 rounded-lg bg-red-900/40 text-red-400 hover:bg-red-900/60 text-sm transition-colors">
              تسجيل الخروج
            </button>
          </div>
        </div>
      ...
      {activeTab === 'security' && (
        <div className="bg-gray-800 rounded-xl border border-gray-700/50 p-5 space-y-4">
          ...
          <div className="space-y-3">
            {[
              { label: 'التحقق بخطوتين (2FA)', desc: 'قادم قريباً', disabled: true },
              { label: 'تشفير HTTPS', desc: 'مفعّل تلقائياً عبر Supabase', disabled: false, active: true },
              { label: 'RLS — Row Level Security', desc: 'مفعّل لجميع الجداول', disabled: false, active: true },
            ].map(item => (
      ...
      {activeTab === 'notifications' && (
        <div className="bg-gray-800 rounded-xl border border-gray-700/50 p-5 space-y-4">
          ...
          <div className="space-y-3">
            {[
              { key: 'whatsapp' as const, label: 'إشعارات واتساب', desc: 'رسائل عند الطلبات الجديدة' },
              ...
              <div key={item.key} className="flex items-center justify-between p-3 rounded-lg bg-gray-900/50">
                ...
                <button onClick={() => setNotifSettings(n => ({ ...n, [item.key]: !n[item.key] }))} className={`px-3 py-1.5 rounded-lg text-xs transition-all ${notifSettings[item.key] ? 'bg-green-900/40 text-green-400' : 'bg-gray-700 text-gray-500'}`}>
      ...
      {activeTab === 'backup' && (
        <div className="bg-gray-800 rounded-xl border border-gray-700/50 p-5 space-y-4">
          ...
          <p className="text-xs text-gray-400">تنزيل نسخة من جميع البيانات (طلبات، تقييمات، أعمال، إعدادات) من Supabase</p>
          <button onClick={handleExport} disabled={exporting} className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-amber-600 hover:bg-amber-500 text-gray-900 text-sm font-medium disabled:opacity-60">
            {exporting ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />}
            تنزيل نسخة احتياطية
          </button>
          <p className="text-xs text-gray-500">Supabase يحتفظ بنسخ احتياطية تلقائية في الخلفية.</p>
        </div>
      )}
