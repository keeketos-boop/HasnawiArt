...
import { getStoredServices, saveService, deleteService } from '@/lib/storage';
import { Service } from '@/types';
import { Plus, Edit2, Trash2, Archive, Eye, X, Check, Upload } from 'lucide-react';
import { toast } from 'sonner';

const EMPTY_SERVICE: Omit<Service, 'id'> = {
  title: '', description: '', details: [''], duration: '', priceRange: '',
  category: 'photography', coverImage: '', requiresDeposit: true, isArchived: false,
};

export default function AdminServicesPage() {
  const storedServices = getStoredServices();
  const storedIds = new Set(storedServices.map(s => s.id));
  const allServices: Service[] = [...storedServices, ...SERVICES.filter(s => !storedIds.has(s.id))];
  ...
  const handleSave = () => {
    ...
    saveService(service);
    toast.success(editService ? 'تم التعديل' : 'تم الإضافة');
    setShowForm(false);
  };
  ...
  const handleArchive = (s: Service) => {
    saveService({ ...s, isArchived: !s.isArchived });
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400">{allServices.length} خدمة</p>
        </div>
        ...
        {allServices.map(service => (
          <div key={service.id} className={`bg-gray-800 rounded-xl border border-gray-700/50 overflow-hidden ${service.isArchived ? 'opacity-60' : ''}`}>
            <div className="relative h-40">
              ...
              {service.isArchived && (
                <div className="absolute top-2 right-2 px-2 py-0.5 bg-gray-800/90 text-gray-400 text-xs rounded-full">مؤرشف</div>
              )}
            </div>
            <div className="p-4">
              ...
              <div className="flex items-center justify-between text-xs mb-3">
                <span className="text-amber-400">{service.priceRange}</span>
      ...
      {preview && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={() => setPreview(null)}>
          <div className="bg-gray-800 rounded-2xl p-5 max-w-md w-full border border-gray-700 space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <img src={preview.coverImage} alt="" className="w-full h-48 object-cover rounded-xl" />
            ...
            <div className="flex gap-4 text-sm"><span className="text-gray-500">المدة: <span className="text-gray-300">{preview.duration}</span></span><span className="text-gray-500">السعر: <span className="text-amber-400">{preview.priceRange}</span></span></div>
